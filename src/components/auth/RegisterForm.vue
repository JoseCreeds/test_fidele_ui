...
const resetData = () => {
    ...
    Object.keys(touched.value).forEach((key) => (touched.value[key as keyof typeof touched.value] = false));
...
function validate(values: any, { setErrors }: any) {
    ...
    return authStore.register(registerData.value).then(() => {
        resetData()
...
<template>
    ...
    <Form @submit="validate" v-slot="{ errors, isSubmitting }" class="mt-5">
      ...
      <v-label class="text-subtitle-1 font-weight-medium pb-2">Email </v-label>
        <VTextField v-model="email"   :rules="touched.email ? emailRules : []"
      required
      @blur="touched.email = true"></VTextField>
  ...
  <v-label class="text-subtitle-1 font-weight-medium pb-2">Rôle</v-label>
                                ...
                                <v-select v-model="roleUser" :items="['Client', 'Vendeur']" variant="outlined" class="text-body-1" hide-details></v-select>
                            </v-col>
                            <v-col v-if="roleUser==='Vendeur'" cols="12" sm="6">
                                 <v-label class="text-subtitle-1 font-weight-medium pb-2">ID kkiapy</v-label>
        <VTextField v-model="idPay" :rules="fnameRules" required ></VTextField>
        ...
        <v-btn size="large" :loading="isSubmitting" color="primary" :disabled="valid" block type="submit" flat>S'inscrire</v-btn>
