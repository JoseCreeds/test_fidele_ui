...
const page = ref({ title: 'Gestion des commandes' });
...
onMounted(async () => {
    await adminStore.getOrdersList();
    FinalInvoice.value = [...getOrdersList.value];

});

const getOrdersList = computed(() => adminStore.orderList);
let FinalInvoice = ref([...getOrdersList.value]);
...
<template>
    ...
    <v-card elevation="10">
        <v-card-item>
            ...
            <div v-else-if="!FinalInvoice || FinalInvoice.length === 0" class="no-data-message ">
                <UiParentCard :title="'Liste des commandes'">
    ...
    <v-dialog v-model="isProductDialogVisible" max-width="60%">
        <v-card>
