...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const addToCart = (product:any) => {
      const existingProduct = cart.value.find((item: any) => item.libelle === product.libelle);
...
<template>
  <v-app-bar elevation="3" flat>
    ...
    <v-container class="d-flex align-center justify-center">
      ...
      <!-- Hamburger icon for mobile view -->
        ...
        <div v-if="isMobileDevice" style="margin-right: 10px;" class="position-relative d-inline-block">
          <RouterLink to="/panier">
  ...
  <main style="margin: 80px 10% 50px;" >
    ...
    <v-row class="justify-content-end mt-5">
        <v-col cols="12">
            <v-row>
                <v-col cols="12" md="4" sm="6" v-for="(card, index) in FinalInvoice" :key="index" >


                    <v-card elevation="10" class="card-hover overflow-hidden" >

                        <v-avatar size="220" class="rounded-0 w-100">
                            <img :src="`${imagePath}/storage/product_images/${card.picture}`" alt="gallery" width="100%"  />
                        </v-avatar>
                        ...
                        <v-card-text>
                          <v-row class="">
                            ...
                            <v-col cols="6" class="d-flex justify-content-end" style="justify-content: right; margin-top: 10px; " >
                              <v-btn color="primary" variant="flat" @click="addToCart(card)">Ajouter</v-btn>
                            </v-col>
                            </v-row>
                        </v-card-text>
                    </v-card>
